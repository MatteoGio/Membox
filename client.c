/* client.c by Giorgi Matteo, 517183 & Quarta Andrea, 517881 */
...
static void
use(const char * filename)
{
	fprintf(stderr,
		"use:\n"
		" %s -l unix_socket_path -c keyid:op:size [-c keyid:op:size] [-s milliseconds] -h\n"
		"   op: %d PUT, %d UPDATE, %d GET, %d REMOVE, %d LOCK, %d UNLOCK\n"
		...
		filename, PUT_OP, UPDATE_OP, GET_OP, REMOVE_OP, LOCK_OP, UNLOCK_OP);
}


static void
init_data(membox_key_t *key, char *data, unsigned int len)
{
	long *p = (long*)data;
	for(int i=0;i<(len/sizeof(unsigned long));++i)
		setKey((membox_key_t*)&p[i], key);
...
static int
check_data(membox_key_t *key, char *data, unsigned int len)
{
	long *p = (long*)data;
	for(int i=0;i<(len/sizeof(unsigned long));++i)
		if (p[i] != *key) return -1;
...
static int
execute_op(int connfd, operation_t *o)
{
	...
	if(op == PUT_OP || op == UPDATE_OP){
		if(size == 0){
			fprintf(stderr,
				"per le operazioni di put/update la size deve essere > 0\n");
			return -1;
		}
	}
	else size = 0;
	...
	switch(msg.hdr.op){
		...
		case OP_PUT_ALREADY:
			fprintf(stderr,
					"PUT: l'oggetto con chiave %ld e' gia' in mbox\n", key);
			return -OP_PUT_ALREADY;
		
		case OP_PUT_TOOMANY:
			fprintf(stderr,
					"PUT: troppi oggetti nel repository\n");
			return -OP_PUT_TOOMANY;

		case OP_PUT_SIZE:
			fprintf(stderr,
					"PUT: l'oggetto con chiave %ld e' troppo grande\n", key);
			return -OP_PUT_SIZE;

		case OP_PUT_REPOSIZE:
			fprintf(stderr,
					"PUT: il repository ha raggiunto la size massima\n");
			return -OP_PUT_REPOSIZE;

		case OP_GET_NONE:
			fprintf(stderr,
					"GET: l'oggetto con chiave %ld non e' presente in mbox\n",
					key);
			return -OP_GET_NONE;

		case OP_REMOVE_NONE:
			fprintf(stderr,
					"REMOVE: l'oggetto con chiave %ld non è presente in mbox\n",
					key);
			return -OP_REMOVE_NONE;

		case OP_UPDATE_SIZE:
			fprintf(stderr,
					"UPDATE: l'oggetto con chiave %ld ha una size non valida\n",
					key);
			return -OP_UPDATE_SIZE;

		case OP_UPDATE_NONE:
			fprintf(stderr,
				"UPDATE: l'oggetto con chiave %ld non e' presente in mbox\n",
				key);
			return -OP_UPDATE_NONE;

		case OP_LOCKED:
			fprintf(stderr,
					"Operazione non permessa, sistema in stato di lock\n");
			return -OP_LOCKED;

		case OP_LOCK_NONE:
			fprintf(stderr,
					"UNLOCK: repository non in stato di lock\n");
			return -OP_LOCK_NONE;
